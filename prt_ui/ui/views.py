...
def ajax_create_process(request):
    if request.method == "POST" and request.is_ajax():
        ...
        socket = create_zmq_connection("127.0.0.1", "5553", zmq.REQ)
        socket.send_json(["prm", "create_process", ["processes", "queues", "logging_q"], {"site": site}])
        resp = socket.recv_json()
...
def ajax_start_workers_for_release(request):
    try:
        if request.method == "POST" and request.is_ajax():
            ...
            socket = create_zmq_connection("127.0.0.1", "5553", zmq.REQ)
            socket.send_json(["prm", "start_workers_for_release", ["processes", "queues", "logging_q"], {'numOfWorkers': numOfWorkers}])
            resp = socket.recv_json()
            socket.close()
